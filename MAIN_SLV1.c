...
//----------------------------- INTERRUPCION SPI -------------------------------     
   ...
   if(SSPIF == 1){               // REVIDAMOS SI HEMOS RECIBIDO ALGUN DATO.      
        ...
        if(temporal == 1){
            spiWrite(V);         // ENVIAMOS EL DATOS DE "V" AL MASTER.
        }
        if(temporal == 0){     
            spiWrite(PORTD);     // ENVIAMOS EL DATO DEL PORTB AL MASTER.
        }
...
//------------------- CONFIGURACION DE ENTRADAS Y SALIDAS ----------------------
    ...
    TRISB = 0x3;             // DECLARAMOS EL PIN 0 Y 1 DEL PORTB COMO ENTRADAS.    
    TRISD = 0x00;
